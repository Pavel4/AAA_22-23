...
class MyRandomForestClassifier:
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_estimators):
            indexes_bootstrap = np.random.choice(indexes, size=X.shape[0], replace=True)
            X_bootstrap = X[indexes_bootstrap]
            y_bootstrap = y[indexes_bootstrap]
            dt = MyDecisionTreeClassifier(max_depth=None, max_features=self.max_features, classes=self.classes)
    ...
    def predict_proba(self, X):
        preds_matrix = np.zeros(shape=(X.shape[0], self.n_classes, self.n_estimators))

        for i, dt in enumerate(self.forest):
            preds_matrix[:, :, i] = dt.predict_proba(X)

        preds = np.sum(preds_matrix, axis=2) / self.n_estimators
